...
import torch
# [ljs patch 2026-09-22] rotary cos/sin cache is rebuilt when the input device changes (MPS/CPU switching bug).
...
class RotaryPositionalEmbedding(nn.Module):
    ...
    def _update_cos_sin_cache(self, x, seq_len):
        if (self.cos_cached is None or self.cos_cached.device != x.device) or seq_len != self.seq_len_cached:
