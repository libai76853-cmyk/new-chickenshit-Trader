...
> **项目状态：已封存（2026-09-23）。** 三个阶段在同一个无幸存者偏差的框架下全部为零，结论见下文「总结论」与 docs/03 到 06。代码与数据流水线保留，可复用于检验他人的回测。
...
阶段 1（基线）完成，结论：没有 alpha（见下）。阶段 2（Kronos 序列特征）完成，结论：Kronos-small 零样本预期收益在干净窗口（2024-07 → 2026-09）RankIC 0.0086、t 0.60，控制动量后残差 0.0003，是反转信号的复述；波动率预测能力真实（RankIC 0.40）但不及 20 日历史波动（0.42）；模型预训练覆盖期（2022-01 → 2024-06）RankIC -0.0082，无泄漏也无信号。不并入模型。见 [docs/05](docs/05_阶段2_Kronos序列特征.md)。阶段 3（SEC 8-K 事件 + Laya 文本）完成，结论：2022 到 2026 年标普 500 在 8-K 公告后 5 日无任何显著漂移、PEAD 不存在（29,347 个事件）；Laya 零样本读 5,022 份 8-K 正文，方向判断与当日市场反应相关仅 0.016、对后 5 日 0.0067，不能作为特征提取器。见 [docs/06](docs/06_阶段3_8K事件与Laya文本特征.md)。

**三个阶段的总结论：标普 500 日频、公开数据、公开模型，没有 alpha。** 价量因子（Alpha158）、金融序列基础模型（Kronos）、公告文本（8-K + Laya）在同一个无幸存者偏差的框架下全部为零。继续的路在框架之外（事件驱动的分钟级执行、小盘股股票池、用市场反应标签微调读者），见 docs/06 第 5.4 节。
