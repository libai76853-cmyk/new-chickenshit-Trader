...
阶段 1（基线）完成，结论：没有 alpha（见下）。阶段 2（Kronos 序列特征）完成，结论：Kronos-small 零样本预期收益在干净窗口（2024-07 → 2026-09）RankIC 0.0086、t 0.60，控制动量后残差 0.0003，是反转信号的复述；波动率预测能力真实（RankIC 0.40）但不及 20 日历史波动（0.42）。不并入模型。见 [docs/05](docs/05_阶段2_Kronos序列特征.md)。
